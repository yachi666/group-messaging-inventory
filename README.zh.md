...
该 smoke 会把 OpenAI-compatible adapter 指向不可用的本地 provider endpoint，然后验证 API 返回带 public `errors` 摘要的 `Failed` run，且 Postgres 中存在结构化 `errors_json` 与 `analysis_run_failed` audit event。AI Template Analysis 工作台会消费这份摘要，在 re-analysis run 失败时让 reviewer 看到失败类别，不暴露 raw provider payload；详细 provider evidence 仍保留在 Postgres 供审计和排障使用。
