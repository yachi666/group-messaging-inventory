...
- Protected command handlers resolve maker/checker/reviewer identity from the authenticated `x-actor-id` header before calling the service layer. Request-body actor fields are treated as backwards-compatible client hints and are overridden by the server-side auth context, preventing simple actor spoofing in audit records and maker-checker checks.
