...
- Tenant/workspace scope is applied before returning API-backed Analysis Result, Review Task, Change Request, Audit Event, and Evidence Package reads. Missing tenant scope remains unrestricted for local compatibility, while gateway deployments should always provide trusted tenant scopes.
...
- `GET /analysis-runs/{runId}/evidence-package` exposes a tenant-scoped single-run evidence package with the public Analysis Run response and related audit events. It supports both successful and failed runs, while provider failures keep raw details out of the public package.
