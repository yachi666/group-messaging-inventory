...
- `npm run test:release-readiness:api:pg` starts the API with Postgres, records release evidence through `POST /analysis-evaluations/release-evidence`, then invokes the same live `check:release-readiness` deployment gate and verifies both the passing path and a version-mismatch blocking path.
...
- `GET /templates/analysis-results` accepts a bounded `limit` query so workbench projections do not require unbounded scans.
- `GET /templates/analysis-results` also applies tenant/workspace scope when `x-gmi-scope-tenants` is present.
...
- `npm run test:release-preflight:local` is the Docker-backed release-candidate gate. It starts local Postgres and Temporal, applies migrations, runs repository and seeded API checks, persists evaluation/release evidence through Postgres and the API, executes the live release-readiness deployment gate, and exercises both successful and provider-failure API -> Temporal -> worker -> Postgres evidence loops. `.github/workflows/release-preflight.yml` exposes the same gate as a manual workflow, and `npm run test:ci-workflow` verifies that this preflight entrypoint keeps the required infrastructure-backed checks.
...
- API access logs are emitted as single-line JSON with `event=http_request`, `requestId`, `actorId`, `roleCount`, `tenantScopeCount`, HTTP method, path, status code, and duration. Counts are logged instead of role/scope values to keep authorization details compact while preserving enough signal to spot anonymous, under-authorized, or unscoped calls.
...
- `/metrics` exposes Prometheus-style API request counters, duration sums, and low-cardinality domain counters for analysis submissions, confirmations, and release evidence records. Metric labels stay intentionally low-cardinality so governed object ids, run ids, release ids, and Change Request ids remain in logs/evidence packages instead of exploding metric series cardinality.
- API rate limiting is opt-in through `API_RATE_LIMIT_ENABLED=true`. When enabled, the API applies an in-memory fixed-window limit by actor id when authenticated, otherwise by client address, and groups high-cardinality paths into low-cardinality route groups such as `analysis-runs`, `change-requests`, `review-tasks`, and `model-configuration`. Operational endpoints `/health`, `/ready`, and `/metrics` are excluded so probes do not consume business capacity.
- `/health` is the liveness endpoint; `/ready` reports API, database, workflow, and AI-provider readiness/configuration state without requiring local Docker infrastructure. Required external dependencies should be probed directly with bounded lightweight checks, for example Postgres `select 1`, a Temporal connection check, and in production preflight `AI_PROVIDER_READINESS_MODE=connectivity` against the configured OpenAI/OpenAI-compatible provider `/models` endpoint. Local development keeps provider readiness in configuration-only mode so health checks do not accidentally spend tokens or depend on external network access.
- Protected API routes use a replaceable governance authorization boundary. Local development uses `API_AUTH_MODE=header`, with required `x-actor-id`, comma-separated `x-gmi-roles`, and optional `x-gmi-scope-tenants` headers for protected routes. Production-oriented gateway integration uses `API_AUTH_MODE=gateway`, reads trusted identity from configurable gateway headers such as `x-gmi-authenticated-actor`, `x-gmi-authenticated-roles`, and `x-gmi-authenticated-tenant-scopes`, and normalizes that identity and tenant scope into the same internal command context without changing controller-level role requirements.
...
- Tenant/workspace scope is applied before returning API-backed Analysis Result, single Analysis Run, Review Task, Change Request, Audit Event, and Evidence Package reads. Missing tenant scope remains unrestricted for local compatibility, while gateway deployments should always provide trusted tenant scopes.
- The audit ledger is exposed through `/audit-events`, filtered by object, source run, change request, limit, and tenant scope. Evidence packages use the same event shape, so review workflows and ledger export share one contract without bypassing data scope.
- `GET /analysis-runs/{runId}` and `GET /analysis-runs/{runId}/evidence-package` expose tenant-scoped single-run reads. The evidence package includes the public Analysis Run response and related audit events. It supports both successful and failed runs, while provider failures keep raw details out of the public package.
...
- Administration -> Model Configuration exposes the same provider contract in the frontend: operators can choose `noop`, OpenAI Agents SDK, an OpenAI-compatible gateway, or the DeepSeek preset; non-secret draft settings are kept in browser local storage, API keys are kept only in browser session storage, and the screen generates the env block that must be applied to API and worker processes before restart.
- `GET /model-configuration/runtime` returns the currently deployed provider runtime in sanitized form: provider, model, prompt version, base URL, readiness mode, retry settings, and credential-present booleans only. It does not return API keys.
- `POST /model-configuration/validate` accepts a candidate provider configuration, performs a bounded one-time `/models` connectivity check for OpenAI/OpenAI-compatible providers, and returns a redacted validation result. Candidate secrets are not persisted and are not included in the response.
- `POST /template-versions/{versionId}/analysis-runs` returns the existing run for duplicate `idempotency-key` submissions, including the run's current status and the same Temporal workflow id. The API reports `workflow.started=false` when the workflow already exists, protecting the durable evidence loop from duplicate workflow starts and duplicate output/audit writes.
...
- `npm run test:harness:temporal:provider-failure` verifies the API -> Temporal -> worker -> Postgres failure path with an unavailable OpenAI-compatible provider endpoint, asserting API `Failed` status with a public `errors` summary, `/audit-events` readback for the `analysis_run_failed` ledger entry, structured `errors_json`, and zero analysis outputs. It also retries the same `idempotency-key` after failure to prove failed terminal runs are reused without duplicate workflow starts or duplicate failure evidence. The AI Template Analysis workbench displays that public summary for failed re-analysis runs, while detailed provider evidence remains in Postgres for audit/debug workflows.
...
- Implemented sanitized model runtime visibility and one-time candidate provider validation through `GET /model-configuration/runtime` and `POST /model-configuration/validate`, with frontend wiring and smoke coverage that proves runtime/candidate API keys are not returned.
- Implemented opt-in API rate limiting with standard `rate_limited` responses, retry headers, actor/IP isolation, operational endpoint bypass, fixture-backed local smoke coverage, and inclusion in no-infrastructure and release-preflight gates.
...
- Implemented tenant-scoped `GET /analysis-runs/{runId}` for both in-memory and Postgres repositories, with backend and seeded Postgres API smoke coverage that blocks known run ids outside the caller tenant scope.
...
- Implemented `npm run test:release-readiness:api:pg` and added it to release preflight so the deployment gate is exercised against a live API and persisted Postgres release evidence before promotion.
...
- Implemented `npm run test:release-preflight:local` plus a manual GitHub Actions release-preflight workflow for Docker-backed Postgres, Temporal, release evidence API, model-configuration and rate-limit API validation, seed verification, and success/failure Temporal harness checks before promotion.
