...
- Review Queue Discovery reads open template review tasks from `/review-tasks`, calls the transition API for claim/start/resolve actions, and falls back to the local discovery queue when the API is unavailable.
