...
- Implemented `npm run test:release-preflight:local` plus a manual GitHub Actions release-preflight workflow for Docker-backed Postgres, Temporal, release evidence API, model-configuration API validation, seed verification, and success/failure Temporal harness checks before promotion.
