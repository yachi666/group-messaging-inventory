...
- `npm run test:backend` verifies that protected routes reject both missing-role and missing-actor requests in header auth mode and that gateway mode overrides spoofed local actor headers with trusted gateway identity.
