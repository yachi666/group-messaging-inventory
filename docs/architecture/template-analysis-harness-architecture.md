...
- Review Queue Discovery, My Tasks, and Completed tabs read status-filtered template review tasks from `/review-tasks`, call the transition API for claim/start/resolve actions, and fall back to local queue data when the API is unavailable.
